...
abstract ColorCode(UInt) from UInt to UInt {
	...
	@:to public static inline function toString(c:ColorCode) {
		var h : String = StringTools.hex(c);
		var rgba = c.toRgba();
		while (h.length < (rgba.a==0 ? 6 : 8))
			h="0"+h;
		return '#$h ( R: ${Math.round(rgba.r*100)/100},  G: ${Math.round(rgba.g*100)/100},  B: ${Math.round(rgba.b*100)/100},  A:${Math.round(rgba.a*100)/100} )';
	}
	...
	@:from public static inline function fromRgb(c:{r:Float, g:Float, b:Float}) : ColorCode {
		return new ColorCode( (Std.int(c.r*255) << 16) | (Std.int(c.g*255)<<8 ) | Std.int(c.b*255) );
	}
	...
	@:from public static inline function fromRgba(c:{r:Float, g:Float, b:Float, a:Float}) {
		return new ColorCode( (Std.int(c.a*255) << 24) | (Std.int(c.r*255) << 16) | (Std.int(c.g*255)<<8 ) | Std.int(c.b*255) );
	}

...
class Buffer {
	...
	public function new(w,h,up, fl_transp:Bool, col:ColorCode, ?useScale2x=false, ?pixelRatio=1.0) {
		width = w;
		height = h;
		upscale = up;
	...
	public static function makeMosaic(wid:Float, ?white=1.0, ?black=1.0) {
		...
		var w = Color.addAlpha(0xE0E0E0, white);
		var b = Color.addAlpha(0x0, black);
		...
		var w = Color.addAlpha(0xFFFFFF, white);
		for(y in 1...wid-1) {
			bd.setPixel32(0, y, w);
	...
	public static function makeMosaic2(wid:Float, ?col=0xffffff, ?alpha=1.0) {
		...
		var w = Color.addAlpha(col, alpha);
